-- SIMPLE SEED DATA FOR THE ROCK SALT
-- Run this in Supabase SQL Editor
-- Creates a basic org first, then adds bands with new migration fields

-- First, create or get the main org
insert into public.orgs (name, slug, website)
values ('The Rock Salt', 'the-rock-salt', 'https://therocksalt.com')
on conflict (slug) do update set name = excluded.name
returning id;

-- Store the org_id in a temp variable (we'll use a DO block)
do $$
declare
  main_org_id uuid;
begin
  -- Get or create the org
  select id into main_org_id from public.orgs where slug = 'the-rock-salt';
  ...
  -- Insert Band Photos (new from migration!)
  ...
  -- Insert Sample Tracks (new from migration!)
  ...
  insert into public.venues (name, slug, city, state, address, website, capacity, org_id) values
    ('Kilby Court', 'kilby-court', 'Salt Lake City', 'UT', '741 S Kilby Ct, Salt Lake City, UT 84102', 'https://kilbycourt.com', 200, main_org_id),
    ('The Urban Lounge', 'the-urban-lounge', 'Salt Lake City', 'UT', '241 S 500 E, Salt Lake City, UT 84102', 'https://theurbanloungeslc.com', 400, main_org_id),
    ('The State Room', 'the-state-room', 'Salt Lake City', 'UT', '638 S State St, Salt Lake City, UT 84111', 'https://thestateroomslc.com', 300, main_org_id),
    ('The Complex', 'the-complex', 'Salt Lake City', 'UT', '536 W 100 S, Salt Lake City, UT 84101', 'https://thecomplexslc.com', 1000, main_org_id),
    ('Velour Live Music Gallery', 'velour', 'Provo', 'UT', '135 N University Ave, Provo, UT 84601', 'https://velourlive.com', 250, main_org_id)
  on conflict (slug) do update set
    address = excluded.address,
    website = excluded.website,
    capacity = excluded.capacity;

  -- Insert Events with new migration fields
  ...
  select
    ...
    true
  from public.venues v where v.slug = 'the-urban-lounge'
  union all
  select
    ...
    null,
    true
  from public.venues v where v.slug = 'kilby-court'
  union all
  select
    'Math Rock Marathon',
    v.id,
    ...
    null,
    false
  from public.venues v where v.slug = 'the-state-room'
  on conflict do nothing;
