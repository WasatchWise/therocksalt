-- SEED DATA - NO CONFLICTS VERSION
-- Org ID: 4cadb578-b415-461f-8039-1fd02f68a030

-- Insert Genres (only if not exists)
insert into public.genres (name)
select 'Rock' where not exists (select 1 from public.genres where name = 'Rock')
union all select 'Punk' where not exists (select 1 from public.genres where name = 'Punk')
union all select 'Indie Rock' where not exists (select 1 from public.genres where name = 'Indie Rock')
union all select 'Emo-core' where not exists (select 1 from public.genres where name = 'Emo-core')
union all select 'Math Rock' where not exists (select 1 from public.genres where name = 'Math Rock')
union all select 'Experimental' where not exists (select 1 from public.genres where name = 'Experimental')
union all select 'Post-Hardcore' where not exists (select 1 from public.genres where name = 'Post-Hardcore')
union all select 'Alternative' where not exists (select 1 from public.genres where name = 'Alternative')
union all select 'Garage Rock' where not exists (select 1 from public.genres where name = 'Garage Rock');

-- Upsert Bands (delete and re-insert to update)
delete from public.bands where slug in ('the-red-bennies', 'form-of-rocket', 'starmy', 'the-new-transit-direction', 'the-brobecks', 'iceburn');
...
select * from (
  select
    b.id as band_id,
    'https://images.unsplash.com/photo-1501386761578-eac5c94b800a?w=1200&h=800&fit=crop' as url,
    'Live at Kilby Court 2024' as caption,
    'unsplash' as source,
    true as is_primary,
    1 as photo_order
  from public.bands b
...
where not exists (
  ...
  where bp.band_id = photos.band_id and bp.photo_order = photos.photo_order
);
...
select * from (
  select
    b.id as band_id,
    'Breaking Point' as title,
    'Our latest single - recorded at June Audio' as description,
    'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3' as file_url,
    'single' as track_type,
    true as is_featured,
    142 as play_count,
    240 as duration_seconds
  from public.bands b
  where b.slug = 'the-new-transit-direction'
  union all
  select
...
where not exists (
  ...
  where bt.band_id = tracks.band_id and bt.title = tracks.title
);

-- Upsert Venues
delete from public.venues where slug in ('kilby-court', 'the-urban-lounge', 'the-state-room', 'the-complex', 'velour');
...
select
  ...
  v.id,
  '2025-11-15T20:00:00Z'::timestamptz,
  '2025-11-15T23:30:00Z'::timestamptz,
...
select
  ...
  v.id,
  '2025-12-10T19:00:00Z'::timestamptz,
  '2025-12-10T23:00:00Z'::timestamptz,
...
select
  'Math Rock Marathon',
  v.id,
  '2025-11-22T18:00:00Z'::timestamptz,
  '2025-11-23T01:00:00Z'::timestamptz,
...
-- Upsert Episodes
delete from public.episodes where slug in ('801-and-done-1', 'slc-sounds-winter-2024', 'emocore-era');
